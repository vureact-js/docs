...
当在处理 [KeepAlive](../adapter-components/keep-alive.md) 组件时，我们通常想要保持路由组件活跃，而不是 RouterView 本身。为了实现这个目的，我们可以将 KeepAlive 组件放置在 `customRender` 内：
...
我们也可以使用一个 [Transition](../adapter-components/transition)  组件来实现在路由组件之间切换时实现过渡效果：
