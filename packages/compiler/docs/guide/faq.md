...
**A:** 当前实验版支持 Vue 3 + `<script setup>` 的核心特性：

| 特性             | 支持状态    | 说明                                       |
| ---------------- | ----------- | ------------------------------------------ |
| Composition API  | ✅ 完全支持 | `ref`, `computed`, `watch`, `provide` 等   |
| `<script setup>` | ✅ 完全支持 | 推荐使用此语法                             |
| 内置组件         | ✅ 完全支持 | `<keep-alive>`, `<transition>` 等          |
| 模板语法         | ✅ 支持     | 支持指令、插值、事件等                     |
| 样式 Scoped      | ✅ 支持     | 生成带作用域的 CSS                         |
| 组件 Props       | ✅ 支持     | `defineProps` 宏                           |
| 组件 Emits       | ✅ 支持     | `defineEmits` 宏                           |
| 组件 Slots       | ✅ 支持     | `defineSlots` 宏                           |
| 组件 Options     | ⚠️ 部分支持 | `defineOptions` 宏，目前仅接收 `name` 选项 |
| 自定义指令       | ❌ 不支持   | 未有相应的解决方案                         |
...
### Q23: VuReact 支持哪些 Vue 3 特性？

**A:** 完整支持 script setup、Composition API、defineProps/defineEmits/defineSlots、watch/computed 等核心特性。

### Q24: 转换后的性能如何？

**A:** 通过编译时优化和零运行时样式方案，转换后的 React 应用性能接近原生 React 应用。

### Q25: 是否支持 Vue 2 或 Options API？
...
### Q30: 为什么生成的 React 组件名与 Vue 中的不一致？
