...
- **8个页面组件**：仪表盘、客户管理、线索管道、任务看板、通知中心、审批中心、系统设置、登录/注册
...
本轮迭代把 CRM 从“单页面功能演示”提升为“跨页面协同闭环”：

- **通知中心**：支持状态筛选、类型筛选、关键词检索、单条处理、全部已读。
- **审批中心**：支持发起审批、通过/驳回、审批历史追踪与详情查看。
- **业务联动**：
  - 线索金额达到阈值（`>= 100 万`）时自动触发审批流程。
  - 任务进入“阻塞”状态时自动生成协同通知。
  - 仪表盘新增协同摘要（未读通知、待审批、今日处理）并提供快捷入口。

这个设计能同时验证三件事：真实业务链路、转换后可运行性、以及复杂交互场景下的语法覆盖稳定性。
...
│   │   ├── NotificationsCenter.vue # 通知中心 - 协同通知处理
│   │   ├── ApprovalsCenter.vue   # 审批中心 - 审批流处理
