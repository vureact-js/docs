...
| 脚本     | `支持` | 如 `ref` / `watch` / `生命周期` 等 API 会转为 React hook ，顶层规则严格 |
| 样式     | `支持` | 单 style + scoped/module 可用，多 style/cssVars 受限                    |
| Vue 路由 | `支持` | 语法和API有适配                                                         |
...
2. 再按模块看细节：
   - [Script 对照](./conversion-script)
   - [Template 对照](./conversion-template)
   - [Style 对照](./conversion-style)
