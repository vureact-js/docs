...
VuReact（发音 `/vjuːˈriːækt/`）是一套面向 Vue 迁移 React 和混合开发的解决方案，兼具 「用 Vue 写 React」 双重能力的编译工具。
它用于将 Vue 3 SFCs・Scripts・Styles 完整转为原生 React 代码（非运行时桥接），覆盖 `<script setup>` 核心全特性，支持渐进式迁移。
...
- **优先可控**：服务于可控工程场景
- **约定驱动**：需要遵守明确的[编译约定](/guide/specification)
