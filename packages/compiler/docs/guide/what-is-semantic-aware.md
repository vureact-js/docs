...
[语义编译](/guide/semantic-comparison/overview) 不是把 Vue 语法机械替换成 React 语法，而是先理解代码的运行语义（如响应式、生命周期与数据流），再生成符合 React 模型的代码。
