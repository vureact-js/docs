...
**语义驱动编译**不是把 Vue 语法机械替换成 React 语法，而是先理解代码的运行语义（如响应式、生命周期与数据流），再生成符合 React 模型的代码。
...
```tsx
const count = useVRef(0); // 保留响应式语义，与 Vue 的 ref 行为对齐
const inc = useCallback(() => {
  count.value++; // 保持运行时行为
}, [count.value]); // 理解 count 是依赖
```

**重点**：这里不是简单地把 `ref` 重命名为 `useState`，而是保留了“可变响应式引用”的行为语义，同时通过 `useCallback` 对函数进行优化。
...
**重点**：
这里重建的是“数据流协议”，不是单纯把 `v-model` 换成一个属性。同时，编译过程会确保 `ChildPanel` 组件内部定义的如 `defineEmits(['update:modelValue'])` 与父组件传递的 `onUpdateModelValue` 在语义上完全对齐。
...
**重点**：编译器保留的是“谁依赖谁、谁是 fallback”，不是只做语法标签替换。
...
**重点**：这里保留的是“遍历语义 + key 稳定性 + 值/键/索引角色”。

## 4.2 Script：对齐响应式、生命周期、setup 逻辑

脚本层会处理：

- 响应式 API 语义映射（`ref/computed/watch`）
...
当顶层变量的初始值依赖响应式数据时，语义编译会把它重建为 `useMemo`，并补上依赖。
...
```tsx
const profile = { label: 'Count', value: count.value }; // 每次渲染都新建对象
```
...
**重点**：这不是“强行套 Hook”，而是编译器识别到“这个值依赖响应式状态”，因此重建为更稳定的 React 形态。
...
```tsx
const BASE = 'panel'; // 静态提升
const count = useVRef(0);
const options = useMemo(() => ({ key: BASE, value: count.value }), [count.value]);
```

**你可以这样理解**：  
语义编译在帮你做“哪些值应当静态、哪些值应当记忆化”的结构性决策。

## 6. 宏语义：`defineProps`、`defineEmits`、`defineSlots`、`defineExpose`
