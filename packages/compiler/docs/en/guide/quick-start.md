...
<script setup lang="ts">
  ...
  // Besides the special comment above, you can also use the defineOptions macro
  // defineOptions({ name: 'HelloWorld' });

  // defineProps is required to define props
...
<!-- VuReact supports Less and Sass -->
<style scoped>
  .counter-card {
...
// Automatically generated by VuReact based on defineProps and defineEmits
type IHelloWorldType = {
  title?: string;
  onUpdate: (value: number) => number;
};

// Automatically wrapped with memo for optimization
const HelloWorld = memo((props: IHelloWorldType) => {
  // ref/computed converted to equivalent adapter APIs
  const step = useVRef(1);
  const count = useVRef(0);
  const title = useComputed(() => `x${step.value}`);

  // Top-level arrow functions automatically analyzed for dependencies and wrapped with useCallback
  const increment = useCallback(() => {
    count.value += step.value;
    props.onUpdate?.(count.value); // emits conversion
  }, [count.value, step.value, props.onUpdate]);

  // Top-level variables automatically analyzed for dependencies and wrapped with useMemo
...
// Maintains the component export automatically
...
4. `ref` / `computed` are converted to equivalent runtime adapter APIs (`useVRef` / `useComputed`)
